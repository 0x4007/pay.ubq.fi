...
*   **Permit Checker Worker:** Runs in the browser background. Handles multiple message types:
    *   `FETCH_NEW_PERMITS`: Fetches and validates permits for the *connected user* based on wallet address and timestamp.
    *   `FETCH_LEADERBOARD_DATA`: Fetches *all* relevant permits, fetches corresponding user details (`users` table) based on `beneficiary_id`, combines the data, and returns the raw list for aggregation. Does *not* perform validation for this message type.
...
*   **Frontend Component (`DeveloperLeaderboard.tsx`):** Displays the developer XP leaderboard. Uses `useLeaderboardData`.
...
*   **Multicall Aggregation:** Planned for future batch claiming feature (Dashboard view). Currently, only single claims are implemented in `usePermitClaiming`.
...
4.  **Leaderboard Fetching:** Frontend (`DeveloperLeaderboard` mount) -> `useLeaderboardData` -> Worker (`FETCH_LEADERBOARD_DATA`) -> Supabase (Query all permits) -> Supabase (Query users by ID) -> Worker (Combine data) -> Worker (Return combined list) -> Frontend (`useLeaderboardData` receives list, aggregates) -> Frontend (`DeveloperLeaderboard` displays data).
